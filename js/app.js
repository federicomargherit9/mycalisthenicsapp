...
const K_MAX_LIFTS = "mca_max_lifts_v1";
const K_MAX_LIFT_ORDER = "mca_max_lift_order_v1";
...
/* =========================================================
   ...
   ========================================================= */
const state = {
  plans: [], tracks: [], goals: [], calendar: {}, journal: [], techNotes: [], maxLifts: [], maxLiftOrder: [],
...
function loadState() {
  ...
  state.maxLifts = JSON.parse(localStorage.getItem(K_MAX_LIFTS) || "[]");
  state.maxLifts.forEach((e, i) => { if (!e.createdAt) e.createdAt = i; if (e.unit === undefined) e.unit = ""; if (e.note === undefined) e.note = ""; });
  state.maxLiftOrder = JSON.parse(localStorage.getItem(K_MAX_LIFT_ORDER) || "null") || [];
  state.maxLifts.forEach(e => { if (!state.maxLiftOrder.includes(e.exercise)) state.maxLiftOrder.push(e.exercise); });
...
function saveMaxLifts() { localStorage.setItem(K_MAX_LIFTS, JSON.stringify(state.maxLifts)); }
function saveMaxLiftOrder() { localStorage.setItem(K_MAX_LIFT_ORDER, JSON.stringify(state.maxLiftOrder)); }
...
const K_NOTIFY = "mca_notify_rest";
function getNotifySetting() { return localStorage.getItem(K_NOTIFY) === "1" && "Notification" in window && Notification.permission === "granted"; }
function syncNotifyToggleUI() {
  const btn = $("#notifyToggle");
  if (!btn) return;
  const wanted = localStorage.getItem(K_NOTIFY) === "1";
  const on = wanted && "Notification" in window && Notification.permission === "granted";
  btn.classList.toggle("on", on);
  btn.setAttribute("aria-checked", on ? "true" : "false");
}
function notifyRestFinished() {
  if (!getNotifySetting() || !document.hidden || !("serviceWorker" in navigator)) return;
  navigator.serviceWorker.getRegistration().then(reg => {
    if (!reg) return;
    reg.showNotification("⏱ Recupero terminato", {
      body: restTimer.label || "Torna alla scheda per la serie successiva.",
      vibrate: [200, 100, 200],
      tag: "mca-rest-timer",
      renotify: true,
...
/* =========================================================
   VIEW SWITCHING
   ========================================================= */
const VIEW_TITLES = { workout: "MyCalisthenicsApp", goals: "Obiettivi", calendar: "Calendario", journal: "Diario", technotes: "Note tecniche", maxlifts: "Massimali", settings: "Impostazioni", timer: "Timer" };
const K_LAST_VIEW_STATE = "mca_last_view_state_v1";
const RESUME_MAX_AGE_MS = 4 * 60 * 60 * 1000; // only resume if the app was left less than 4h ago

function switchView(view, isRestoring) {
  state.currentView = view;
  ["workout", "goals", "calendar", "journal", "technotes", "maxlifts", "settings", "timer"].forEach(v => {
    $("#view-" + v).classList.toggle("hidden", v !== view);
  });
  $("#drawerAddPlanWrap").classList.toggle("hidden", view !== "workout");
  document.querySelectorAll(".nav-tab").forEach(b => b.classList.toggle("active", b.dataset.view === view));
  if (view === "workout") { $("#topbarTitle").innerHTML = "MyCalisthenics<span>App</span>"; renderWorkout(); if (isRestoring) restoreWorkoutScroll(); }
  else { $("#topbarTitle").textContent = VIEW_TITLES[view]; }
  if (view === "goals") renderGoals();
  if (view === "calendar") renderCalendar();
  if (view === "journal") renderJournal();
  if (view === "technotes") renderTechNotes();
  if (view === "maxlifts") renderMaxLifts();
  if (view === "settings") { renderSettingsPlans(); renderSettingsArchive(); renderSettingsTracks(); renderMusicSections(); syncAutoRestToggleUI(); syncNotifyToggleUI(); }
  closeDrawer();
  if (!isRestoring) saveLastViewState();
}

function saveLastViewState() {
  const scrollEl = $("#view-workout");
  localStorage.setItem(K_LAST_VIEW_STATE, JSON.stringify({
...
function restoreWorkoutScroll() {
  const saved = JSON.parse(localStorage.getItem(K_LAST_VIEW_STATE) || "null");
  if (!saved || saved.planId !== state.currentPlanId) return;
  requestAnimationFrame(() => { const el2 = $("#view-workout"); if (el2) el2.scrollTop = saved.scrollTop || 0; });
}
function restoreLastViewOnLoad() {
  const saved = JSON.parse(localStorage.getItem(K_LAST_VIEW_STATE) || "null");
  if (!saved || !saved.view) return;
  if (Date.now() - (saved.savedAt || 0) > RESUME_MAX_AGE_MS) return;
  if (saved.view !== "workout" && (!$("#view-" + saved.view) )) return;
  switchView(saved.view, true);
}
...
function isItemDone(item) {
  if (item.type === "circuit") {
    const p = state.progress[item.id] || { completedRounds: 0, currentStepIndex: 0 };
    return p.completedRounds >= item.rounds;
  }
  return (state.progress[item.id] || 0) >= item.sets;
}
...
function buildCircuitCard(circuit, phase, exIdxInPhase, plan) {
  const p = state.progress[circuit.id] || { completedRounds: 0, currentStepIndex: 0 };
  const steps = circuit.steps || [];
  const fullyDone = steps.length > 0 && p.completedRounds >= circuit.rounds;
  const card = el("div", "circuit-card" + (fullyDone ? " done" : ""));

  const head = el("div", "circuit-head");
  head.innerHTML = `<div class="circuit-title">${escapeHtml(circuit.name || "Circuito")}</div>`;
  const check = el("button", "exercise-check circuit-check", fullyDone ? "✓" : "");
  check.addEventListener("click", () => {
    if (!steps.length) { toast("Aggiungi almeno un esercizio al circuito dalle impostazioni."); return; }
    const prevSnapshot = state.progress[circuit.id] ? JSON.parse(JSON.stringify(state.progress[circuit.id])) : undefined;
    pushUndoAction(circuit.id, prevSnapshot);

    if (fullyDone) { delete state.progress[circuit.id]; saveProgress(); renderWorkout(); maybeCelebrate(plan); return; }

    p.currentStepIndex += 1;
    let justFinishedRound = false, justFinishedCircuit = false;
    if (p.currentStepIndex >= steps.length) {
      p.completedRounds += 1; p.currentStepIndex = 0; justFinishedRound = true;
      if (p.completedRounds >= circuit.rounds) justFinishedCircuit = true;
    }
    state.progress[circuit.id] = p;
    saveProgress();
    renderWorkout();
    maybeCelebrate(plan);

    if (getAutoRestSetting()) {
      if (justFinishedCircuit) {
        const isLastInPhase = exIdxInPhase === phase.exercises.length - 1;
        if (isLastInPhase) { if (phase.restAfter) startRestTimer(phase.restAfter, "Recupero tra fasi"); }
        else if (circuit.restAfter) startRestTimer(circuit.restAfter, "Recupero tra esercizi");
      } else if (justFinishedRound) {
        if (circuit.restBetweenRounds) startRestTimer(circuit.restBetweenRounds, "Recupero tra giri — " + circuit.name);
      } else if (circuit.restBetweenExercises) {
        startRestTimer(circuit.restBetweenExercises, "Recupero — " + circuit.name);
      }
    }
  });
  ...
  const roundLabel = fullyDone ? circuit.rounds : p.completedRounds + 1;
  const stepLabel = fullyDone ? steps.length : p.currentStepIndex;
  card.appendChild(el("div", "circuit-progress",
    `Giro ${Math.min(roundLabel, circuit.rounds)}/${circuit.rounds} · Esercizio ${stepLabel}/${steps.length} in questo giro`));

  if (steps.length) {
    const stepsRow = el("div", "circuit-steps-row");
    let currentChipEl = null;
    steps.forEach((st, sIdx) => {
      const stepDoneInThisRound = !fullyDone && sIdx < p.currentStepIndex;
      const isCurrent = !fullyDone && sIdx === p.currentStepIndex;
      const chip = el("div", "circuit-step-chip" + (isCurrent ? " current" : "") + (stepDoneInThisRound || fullyDone ? " step-done" : ""));
      chip.innerHTML = `${escapeHtml(st.name || "Esercizio")}${st.reps ? `<span>${escapeHtml(st.reps)}</span>` : ""}`;
      if (isCurrent) currentChipEl = chip;
      stepsRow.appendChild(chip);
    });
    card.appendChild(stepsRow);
    // Keep the current exercise in view automatically as you progress
    // through the circuit, instead of making people hunt for it by hand.
    requestAnimationFrame(() => {
      const target = currentChipEl || stepsRow.lastElementChild;
      if (target && target.scrollIntoView) target.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" });
    });
  } else {
    card.appendChild(el("div", "playlist-empty", "Nessun esercizio nel circuito."));
  }

  if (circuit.restBetweenExercises || circuit.restBetweenRounds) {
    const chipsWrap = el("div", "circuit-rest-chips");
    if (circuit.restBetweenExercises) {
      const chip = el("button", "rest-chip", "⏱ " + fmtRest(circuit.restBetweenExercises) + " tra esercizi");
      chip.addEventListener("click", () => startRestTimer(circuit.restBetweenExercises, "Recupero — " + circuit.name));
      chipsWrap.appendChild(chip);
    }
    if (circuit.restBetweenRounds) {
      const chip = el("button", "rest-chip", "⏱ " + fmtRest(circuit.restBetweenRounds) + " tra giri");
      chip.addEventListener("click", () => startRestTimer(circuit.restBetweenRounds, "Recupero tra giri — " + circuit.name));
      chipsWrap.appendChild(chip);
    }
    card.appendChild(chipsWrap);
  }
  if (circuit.note) card.appendChild(el("div", "exercise-note", escapeHtml(circuit.note)));
...
function renderWorkout() {
  ...
  const doneEx = plan.phases.reduce((s, ph) => s + ph.exercises.filter(e => isItemDone(e)).length, 0);
  ...
  plan.phases.forEach((phase, phIdx) => {
    ...
    phase.exercises.forEach((ex, exIdxInPhase) => {
      if (ex.type === "circuit") {
        phaseEl.appendChild(buildCircuitCard(ex, phase, exIdxInPhase, plan));
        if (ex.restAfter) {
...
function renderSettingsPlans() {
  ...
  groups.forEach((g, gIdx) => {
    ...
    groupPlans.forEach((p) => {
      ...
      const dupBtn = el("button", "icon-mini", "⧉");
      dupBtn.title = "Duplica scheda";
      dupBtn.addEventListener("click", () => {
        const copy = JSON.parse(JSON.stringify(p));
        copy.id = uid("plan");
        copy.title = p.title + " (copia)";
        copy.archived = false;
        copy.phases.forEach(ph => ph.exercises.forEach(e => { e.id = uid("ex"); }));
        state.plans.splice(globalIdx + 1, 0, copy);
        savePlans();
        renderSettingsPlans(); renderDrawer();
      ...
      actions.appendChild(editBtn); actions.appendChild(dupBtn); actions.appendChild(archBtn); actions.appendChild(delBtn);
...
function buildCircuitEditor(circuit, exBlock) {
  const nf = el("div", "field", "<label>Titolo circuito</label>");
  const ni = el("input"); ni.type = "text"; ni.value = circuit.name; ni.placeholder = "Es. Circuito gambe";
  ni.addEventListener("input", () => circuit.name = ni.value);
  nf.appendChild(ni); exBlock.appendChild(nf);

  const row1 = el("div", "field-row");
  const roundsF = el("div", "field", "<label>Giri</label>");
  const roundsI = el("input"); roundsI.type = "number"; roundsI.min = "1"; roundsI.value = circuit.rounds;
  roundsI.addEventListener("input", () => circuit.rounds = Math.max(1, parseInt(roundsI.value, 10) || 1));
  roundsF.appendChild(roundsI);
  const restAfterF = el("div", "field", "<label>Recupero dopo il circuito (s)</label>");
  const restAfterI = el("input"); restAfterI.type = "number"; restAfterI.min = "0"; restAfterI.value = circuit.restAfter || 0;
  restAfterI.addEventListener("input", () => circuit.restAfter = parseInt(restAfterI.value, 10) || 0);
  restAfterF.appendChild(restAfterI);
  row1.appendChild(roundsF); row1.appendChild(restAfterF); exBlock.appendChild(row1);

  const row2 = el("div", "field-row");
  const restBetExF = el("div", "field", "<label>Recupero tra esercizi (s)</label>");
  const restBetExI = el("input"); restBetExI.type = "number"; restBetExI.min = "0"; restBetExI.value = circuit.restBetweenExercises || 0;
  restBetExI.addEventListener("input", () => circuit.restBetweenExercises = parseInt(restBetExI.value, 10) || 0);
  restBetExF.appendChild(restBetExI);
  const restBetRoundsF = el("div", "field", "<label>Recupero tra giri (s)</label>");
  const restBetRoundsI = el("input"); restBetRoundsI.type = "number"; restBetRoundsI.min = "0"; restBetRoundsI.value = circuit.restBetweenRounds || 0;
  restBetRoundsI.addEventListener("input", () => circuit.restBetweenRounds = parseInt(restBetRoundsI.value, 10) || 0);
  restBetRoundsF.appendChild(restBetRoundsI);
  row2.appendChild(restBetExF); row2.appendChild(restBetRoundsF); exBlock.appendChild(row2);

  const stepsWrap = el("div");
  exBlock.appendChild(stepsWrap);

  function renderSteps() {
    stepsWrap.innerHTML = "";
    if (!circuit.steps) circuit.steps = [];
    circuit.steps.forEach((st, sIdx) => {
      const stBlock = el("div", "subblock"); stBlock.style.background = "var(--surface-2)";
      const stHeader = el("div", "subblock-header");
      stHeader.innerHTML = `<span>Esercizio ${sIdx + 1} del circuito</span>`;
      const stActions = el("div", "subblock-header-actions");
      const stReorder = el("div", "reorder-btns");
      const stUp = el("button", "", "▲"); stUp.addEventListener("click", () => {
        if (sIdx <= 0) return;
        const [moved] = circuit.steps.splice(sIdx, 1); circuit.steps.splice(sIdx - 1, 0, moved);
        renderSteps();
      });
      const stDown = el("button", "", "▼"); stDown.addEventListener("click", () => {
        if (sIdx >= circuit.steps.length - 1) return;
        const [moved] = circuit.steps.splice(sIdx, 1); circuit.steps.splice(sIdx + 1, 0, moved);
        renderSteps();
      });
      stReorder.appendChild(stUp); stReorder.appendChild(stDown);
      stActions.appendChild(stReorder);
      const stRm = el("button", "mini-remove", "Rimuovi");
      stRm.addEventListener("click", () => { circuit.steps.splice(sIdx, 1); renderSteps(); });
      stActions.appendChild(stRm);
      stHeader.appendChild(stActions); stBlock.appendChild(stHeader);

      const stNameF = el("div", "field", "<label>Nome esercizio</label>");
      const stNameI = el("input"); stNameI.type = "text"; stNameI.value = st.name || "";
      stNameI.addEventListener("input", () => st.name = stNameI.value);
      stNameF.appendChild(stNameI); stBlock.appendChild(stNameF);

      const stRepsF = el("div", "field", "<label>Ripetizioni / durata</label>");
      const stRepsI = el("input"); stRepsI.type = "text"; stRepsI.value = st.reps || ""; stRepsI.placeholder = "10 rip. / 20s";
      stRepsI.addEventListener("input", () => st.reps = stRepsI.value);
      stRepsF.appendChild(stRepsI); stBlock.appendChild(stRepsF);

      stepsWrap.appendChild(stBlock);
    });
  }
  renderSteps();

  const addStepBtn = el("button", "btn-add", "+ Aggiungi esercizio al circuito");
  addStepBtn.addEventListener("click", () => { circuit.steps = circuit.steps || []; circuit.steps.push({ id: uid("cstep"), name: "", reps: "" }); renderSteps(); });
  exBlock.appendChild(addStepBtn);

  const noteF = el("div", "field", "<label>Nota (opzionale)</label>");
  const noteI = el("input"); noteI.type = "text"; noteI.value = circuit.note || "";
  noteI.addEventListener("input", () => circuit.note = noteI.value);
  ...
  function renderPhases() {
    ...
    plan.phases.forEach((phase, pIdx) => {
      ...
      phase.exercises.forEach((ex, eIdx) => {
        ...
        exHeader.innerHTML = `<span>${ex.type === "circuit" ? "Circuito" : "Esercizio"} ${eIdx + 1}</span>`;
        ...
        const exUp = el("button", "", "▲"); exUp.addEventListener("click", () => {
          if (eIdx > 0) {
            ...
            // first exercise of this phase: move to the end of the previous phase
            const [moved] = phase.exercises.splice(eIdx, 1);
            plan.phases[pIdx - 1].exercises.push(moved);
          } else return;
          renderPhases();
        });
        const exDown = el("button", "", "▼"); exDown.addEventListener("click", () => {
          if (eIdx < phase.exercises.length - 1) {
            const [moved] = phase.exercises.splice(eIdx, 1);
            phase.exercises.splice(eIdx + 1, 0, moved);
          } else if (pIdx < plan.phases.length - 1) {
            // last exercise of this phase: move to the start of the next phase
            const [moved] = phase.exercises.splice(eIdx, 1);
            plan.phases[pIdx + 1].exercises.unshift(moved);
          } else return;
          renderPhases();
        });
      ...
      const addCircuitBtn = el("button", "btn-add", "+ Aggiungi circuito");
      addCircuitBtn.style.marginTop = "8px";
      addCircuitBtn.addEventListener("click", () => {
        phase.exercises.push({ id: uid("circuit"), type: "circuit", name: "", rounds: 3, restBetweenExercises: 15, restBetweenRounds: 60, restAfter: 0, note: "", steps: [] });
        renderPhases();
      });
      block.appendChild(addCircuitBtn);
...
/* =========================================================
   MASSIMALI (personal records) — pill filter chips, recent-entries
   list per exercise, and a hand-rolled scrollable SVG line chart
   (no external chart library, so it still works fully offline).
   ========================================================= */
const MAXLIFT_COLORS = ["#ff4d1c", "#5b8dae", "#7fb069", "#c58bf2", "#e8c547", "#ede7da", "#e15252", "#4dd0c4"];
function colorForExercise(name) {
  let hash = 0;
  for (let i = 0; i < name.length; i++) hash = (hash * 31 + name.charCodeAt(i)) >>> 0;
  return MAXLIFT_COLORS[hash % MAXLIFT_COLORS.length];
}

let selectedMaxLiftExercise = "Tutti";
let maxLiftShowAll = false;
let expandedMaxLiftId = null;
let maxLiftReorderMode = false;

function renderMaxLifts() {
  renderMaxLiftChips();
  renderMaxLiftContent();
}

function renderMaxLiftChips() {
  const row = $("#maxLiftChipRow");
  row.innerHTML = "";
  const allChip = el("button", "chip" + (selectedMaxLiftExercise === "Tutti" ? " active" : ""), "Tutti");
  allChip.addEventListener("click", () => { if (maxLiftReorderMode) return; selectedMaxLiftExercise = "Tutti"; maxLiftShowAll = false; expandedMaxLiftId = null; renderMaxLifts(); });
  row.appendChild(allChip);
  state.maxLiftOrder.forEach((name, idx) => {
    if (!state.maxLifts.some(e => e.exercise === name)) return; // hide exercises with no entries left
    const chip = el("button", "chip" + (selectedMaxLiftExercise === name ? " active" : ""));
    chip.appendChild(document.createTextNode(name));
    chip.addEventListener("click", () => { if (maxLiftReorderMode) return; selectedMaxLiftExercise = name; maxLiftShowAll = false; expandedMaxLiftId = null; renderMaxLifts(); });
    if (maxLiftReorderMode) {
      chip.style.marginTop = "22px";
      const moveWrap = el("div", "chip-move-btns");
      const leftBtn = el("button", "", "◀"); leftBtn.addEventListener("click", (e) => { e.stopPropagation(); moveMaxLiftExercise(idx, -1); });
      const rightBtn = el("button", "", "▶"); rightBtn.addEventListener("click", (e) => { e.stopPropagation(); moveMaxLiftExercise(idx, 1); });
      moveWrap.appendChild(leftBtn); moveWrap.appendChild(rightBtn);
      chip.appendChild(moveWrap);
    }
    row.appendChild(chip);
  });
}
function moveMaxLiftExercise(idx, dir) {
  const newIdx = idx + dir;
  if (newIdx < 0 || newIdx >= state.maxLiftOrder.length) return;
  const [n] = state.maxLiftOrder.splice(idx, 1);
  state.maxLiftOrder.splice(newIdx, 0, n);
  saveMaxLiftOrder();
  renderMaxLiftChips();
}

function renderMaxLiftContent() {
  const wrap = $("#maxLiftContent");
  wrap.innerHTML = "";
  if (selectedMaxLiftExercise === "Tutti") {
    wrap.appendChild(buildCombinedChart());
    return;
  }
  const entries = state.maxLifts.filter(e => e.exercise === selectedMaxLiftExercise)
    .sort((a, b) => b.date.localeCompare(a.date) || (b.createdAt || 0) - (a.createdAt || 0));
  if (!entries.length) {
    wrap.appendChild(el("div", "playlist-empty", "Nessun massimale registrato per questo esercizio."));
    return;
  }
  const visible = maxLiftShowAll ? entries : entries.slice(0, 4);
  visible.forEach(entry => wrap.appendChild(buildPrEntryCard(entry)));
  if (entries.length > 4) {
    const link = el("div", "pr-see-more", maxLiftShowAll ? "Vedi di meno ▲" : `Vedi di più (${entries.length - 4}) ▼`);
    link.addEventListener("click", () => { maxLiftShowAll = !maxLiftShowAll; renderMaxLiftContent(); });
    wrap.appendChild(link);
  }
  wrap.appendChild(buildExerciseChart(selectedMaxLiftExercise, entries));
}

function buildPrEntryCard(entry) {
  const card = el("div", "pr-entry" + (expandedMaxLiftId === entry.id ? " expanded" : ""));
  const head = el("div", "pr-entry-head");
  head.innerHTML = `<span class="pr-entry-date">${fmtDate(entry.date)}</span><span class="pr-entry-value">${entry.value}${entry.unit ? (" " + escapeHtml(entry.unit)) : ""}</span>`;
  head.addEventListener("click", () => { if (!entry.note) return; expandedMaxLiftId = expandedMaxLiftId === entry.id ? null : entry.id; renderMaxLiftContent(); });
  card.appendChild(head);
  if (entry.note) card.appendChild(el("div", "pr-entry-note", escapeHtml(entry.note)));
  const actions = el("div", "n-actions");
  const editBtn = el("button", "", "Modifica"); editBtn.addEventListener("click", () => openMaxLiftForm(entry.id));
  const delBtn = el("button", "", "Elimina");
  delBtn.addEventListener("click", () => {
    if (!confirm("Eliminare questo massimale?")) return;
    state.maxLifts = state.maxLifts.filter(x => x.id !== entry.id);
    saveMaxLifts(); renderMaxLifts();
  });
  actions.appendChild(editBtn); actions.appendChild(delBtn);
  card.appendChild(actions);
  return card;
...
function buildExerciseChart(name, entriesDesc) {
  const points = entriesDesc.slice().sort((a, b) => a.date.localeCompare(b.date));
  return buildLineChartSVG([{ name, color: colorForExercise(name), points }], false);
}
function buildCombinedChart() {
  const names = state.maxLiftOrder.filter(n => state.maxLifts.some(e => e.exercise === n));
  if (!names.length) return el("div", "playlist-empty", "Nessun massimale registrato ancora.");
  const series = names.map(n => ({
    name: n, color: colorForExercise(n),
    points: state.maxLifts.filter(e => e.exercise === n).sort((a, b) => a.date.localeCompare(b.date))
  }));
  return buildLineChartSVG(series, true);
}

function buildLineChartSVG(series, showLegend) {
  const wrap = el("div");
  if (showLegend) {
    const legend = el("div", "chart-legend");
    series.forEach(s => legend.appendChild(el("span", "", `<span class="dot" style="background:${s.color}"></span>${escapeHtml(s.name)}`)));
    wrap.appendChild(legend);
  }

  const allDates = [];
  series.forEach(s => s.points.forEach(p => { if (!allDates.includes(p.date)) allDates.push(p.date); }));
  allDates.sort();
  if (!allDates.length) { wrap.appendChild(el("div", "playlist-empty", "Nessun dato da mostrare ancora.")); return wrap; }

  const pointSpacing = 56, paddingX = 20, chartHeight = 180, paddingYTop = 14, paddingYBottom = 22;
  const width = Math.max(240, paddingX * 2 + (allDates.length - 1) * pointSpacing);

  let allValues = [];
  series.forEach(s => s.points.forEach(p => allValues.push(p.value)));
  let minV = Math.min(...allValues), maxV = Math.max(...allValues);
  if (minV === maxV) { minV -= 1; maxV += 1; }
  const rangePad = (maxV - minV) * 0.12;
  minV -= rangePad; maxV += rangePad;

  const xFor = (idx) => paddingX + idx * pointSpacing;
  const yFor = (v) => paddingYTop + (chartHeight - paddingYTop - paddingYBottom) * (1 - (v - minV) / (maxV - minV));

  let svgInner = "";
  // faint horizontal gridlines
  for (let i = 0; i <= 4; i++) {
    const gy = paddingYTop + (chartHeight - paddingYTop - paddingYBottom) * (i / 4);
    svgInner += `<line x1="0" y1="${gy}" x2="${width}" y2="${gy}" stroke="#3a3527" stroke-width="1"/>`;
  }
  series.forEach(s => {
    const pts = s.points.map(p => `${xFor(allDates.indexOf(p.date))},${yFor(p.value)}`).join(" ");
    svgInner += `<polyline points="${pts}" fill="none" stroke="${s.color}" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>`;
    s.points.forEach(p => { svgInner += `<circle cx="${xFor(allDates.indexOf(p.date))}" cy="${yFor(p.value)}" r="4" fill="${s.color}"/>`; });
  });
  allDates.forEach((d, idx) => {
    const short = d.slice(5).split("-").reverse().join("/");
    svgInner += `<text x="${xFor(idx)}" y="${chartHeight - 4}" font-size="9" fill="#6b6558" text-anchor="middle" font-family="monospace">${short}</text>`;
  });
  ...
  for (let i = 4; i >= 0; i--) {
    ...
    yaxis.appendChild(el("div", "", Math.round(v).toString()));
  }
  chartWrap.appendChild(yaxis);
  const scrollDiv = el("div", "chart-scroll");
  scrollDiv.innerHTML = `<svg width="${width}" height="${chartHeight}" viewBox="0 0 ${width} ${chartHeight}">${svgInner}</svg>`;
  chartWrap.appendChild(scrollDiv);
  wrap.appendChild(chartWrap);

  // Start scrolled to the most recent entries (right edge), like the rest of the app.
  requestAnimationFrame(() => { scrollDiv.scrollLeft = scrollDiv.scrollWidth; });

  return wrap;
}

function openMaxLiftForm(entryId) {
  const editing = !!entryId;
  const entry = editing
    ? { ...state.maxLifts.find(e => e.id === entryId) }
    : { id: uid("pr"), exercise: selectedMaxLiftExercise !== "Tutti" ? selectedMaxLiftExercise : "", date: todayIso(), value: "", unit: "", note: "", createdAt: Date.now() };

  const content = $("#sheetContent");
  content.innerHTML = "";
  content.appendChild(el("h3", "", editing ? "Modifica massimale" : "Nuovo massimale"));

  const exF = el("div", "field", "<label>Esercizio</label>");
  const exI = el("input"); exI.type = "text"; exI.value = entry.exercise; exI.setAttribute("list", "maxLiftExercisePresets"); exI.placeholder = "Es. Push-up, Pull-up, HSPU...";
  const datalist = el("datalist"); datalist.id = "maxLiftExercisePresets";
  state.maxLiftOrder.forEach(n => { const o = document.createElement("option"); o.value = n; datalist.appendChild(o); });
  exF.appendChild(exI); exF.appendChild(datalist); content.appendChild(exF);

  const row1 = el("div", "field-row");
  const dateF = el("div", "field", "<label>Data</label>");
  const dateI = el("input"); dateI.type = "date"; dateI.value = entry.date; dateF.appendChild(dateI);
  const valF = el("div", "field", "<label>Ripetizioni / secondi</label>");
  const valI = el("input"); valI.type = "number"; valI.step = "any"; valI.value = entry.value;
  valF.appendChild(valI);
  row1.appendChild(dateF); row1.appendChild(valF); content.appendChild(row1);

  const unitF = el("div", "field", "<label>Unità (opzionale)</label>");
  const unitI = el("input"); unitI.type = "text"; unitI.value = entry.unit || ""; unitI.placeholder = "rip. / sec / kg";
  unitF.appendChild(unitI); content.appendChild(unitF);

  const noteF = el("div", "field", "<label>Note (max 300 caratteri)</label>");
  const noteI = el("textarea"); noteI.maxLength = 300; noteI.value = entry.note || ""; noteI.style.minHeight = "80px";
  noteF.appendChild(noteI); content.appendChild(noteF);

  const actions = el("div", "sheet-actions");
  const cancelBtn = el("button", "btn-secondary", "Annulla"); cancelBtn.addEventListener("click", closeSheet);
  const saveBtn = el("button", "btn-primary", "Salva");
  saveBtn.addEventListener("click", () => {
    const exName = exI.value.trim();
    if (!exName) { toast("Inserisci il nome dell'esercizio."); return; }
    const val = parseFloat(valI.value);
    if (isNaN(val)) { toast("Inserisci un numero valido."); return; }
    entry.exercise = exName; entry.date = dateI.value || todayIso(); entry.value = val;
    entry.unit = unitI.value.trim(); entry.note = noteI.value.trim().slice(0, 300);
    if (editing) state.maxLifts[state.maxLifts.findIndex(e => e.id === entryId)] = entry; else state.maxLifts.push(entry);
    if (!state.maxLiftOrder.includes(exName)) { state.maxLiftOrder.push(exName); saveMaxLiftOrder(); }
    saveMaxLifts();
    selectedMaxLiftExercise = exName; maxLiftShowAll = false; expandedMaxLiftId = null;
    renderMaxLifts();
    closeSheet(); toast("Massimale salvato.");
  });
  actions.appendChild(cancelBtn); actions.appendChild(saveBtn); content.appendChild(actions);
  openSheet();
}
...
function wireGlobal() {
  ...
  $("#notifyToggle").addEventListener("click", () => {
    const currentlyWanted = localStorage.getItem(K_NOTIFY) === "1";
    if (currentlyWanted) {
      localStorage.setItem(K_NOTIFY, "0");
      syncNotifyToggleUI();
      return;
    }
    if (!("Notification" in window)) { toast("Le notifiche non sono supportate su questo browser."); return; }
    if (Notification.permission === "granted") {
      localStorage.setItem(K_NOTIFY, "1");
      syncNotifyToggleUI();
      toast("Notifiche attivate.");
    } else if (Notification.permission === "denied") {
      toast("Notifiche bloccate dal browser. Abilitale dalle impostazioni del sito per usarle.");
    } else {
      Notification.requestPermission().then(perm => {
        if (perm === "granted") { localStorage.setItem(K_NOTIFY, "1"); toast("Notifiche attivate."); }
        else { localStorage.setItem(K_NOTIFY, "0"); toast("Permesso non concesso."); }
        syncNotifyToggleUI();
      });
    }
  });
  ...
  $("#btnAddMaxLift").addEventListener("click", () => openMaxLiftForm(null));
  $("#maxLiftReorderToggle").addEventListener("click", () => {
    maxLiftReorderMode = !maxLiftReorderMode;
    $("#maxLiftReorderToggle").classList.toggle("on", maxLiftReorderMode);
    renderMaxLiftChips();
  });
...
function exportBackup() {
  const payload = {
    ...
    maxLiftOrder: state.maxLiftOrder,
...
function importBackup(file) {
  ...
  reader.onload = () => {
    ...
    const addedMaxLifts = mergeById(state.maxLifts, data.maxLifts);
    mergeStrings(state.maxLiftOrder, data.maxLiftOrder);
    ...
    savePlans(); saveTracks(); saveGoals(); saveJournal(); saveTechNotes(); saveMaxLifts(); saveMaxLiftOrder(); saveMusicSections(); savePlanGroupOrder(); saveCalendar();
    ...
    toast(`Uniti: +${addedPlans} schede, +${addedTracks} canzoni, +${addedGoals} obiettivi, +${addedNotes} note, +${addedTechNotes} note tecniche, +${addedMaxLifts} massimali, +${addedCalEntries} eventi calendario.`);
  };
...
function init() {
  ...
  restoreLastViewOnLoad();
  wireViewPersistence();
}

function wireViewPersistence() {
  document.addEventListener("visibilitychange", () => { if (document.hidden) saveLastViewState(); });
  window.addEventListener("pagehide", saveLastViewState);
  let scrollSaveTimer = null;
  $("#view-workout").addEventListener("scroll", () => {
    clearTimeout(scrollSaveTimer);
    scrollSaveTimer = setTimeout(saveLastViewState, 600);
  });
}
